...
class MurphyView(QWidget):
    ...
    def toggle_brake_failure(self, status_label):
        ...
        main_window = self.parent()
        if isinstance(main_window, MainWindow):
            main_window.Train.update_emergency_brake_button()

class MetricsTab(QWidget):
    currentSpeed_updated = pyqtSignal(float)
    def __init__(self, Train):
        super().__init__()
    ...
    def update_labels(self):
        ...
        temperature = self.Train.get_temp()
        light_status = "On" if self.Train.lights_status else "Off"
        ...
        self.currentSpeed_updated.emit(currentSpeed)
        print (f"current speed emitted from trainapp, current speed: {currentSpeed}")
...
class TestBench(QWidget):
    def __init__(self, Train, metrics_tab, train_tab):
        ...
        # Power Command
        power_command_label = QLabel("Power Command (Watts) NOT CURRENTLY FUNCTIONING:")
    ...
    def update_inputs(self):
        x = self.power_command_input.setText(f"{self.Train.get_power_output():.2f}")
        print(f"current power out: {x}")
        self.temperature_input.setText(f"{self.Train.get_temp():.2f}")
        self.authority_input.setText(f"{self.Train.get_authority():.2f}")
    ...
    def set_command_speed(self):
        ...
        self.Train.set_commanded_speed(command_speed, self.Train.cs_driver, self.Train.speed_limit)
        self.metrics_tab.update_labels()
        print(command_speed)
    ...
    def toggle_lights(self):
        self.Train.internalLightToggle()
        self.metrics_tab.update_labels()
    ...
    def toggle_service_brake(self):
        self.Train.toggle_service_brake()
        self.metrics_tab.update_labels()
