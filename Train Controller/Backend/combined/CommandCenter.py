...
class CommandCenter(QWidget):
    ...
    def __init__(self):
        ...
        self.backend = Train()
        #self.backend.doors_changed.connect(self.update_values)
    ...
    """)
        ...
        self.sbrake_label = QLabel(f"Service Brake Status: {self.backend.serviceBrake}")
        self.ebrake_label = QLabel(f"Emergency Brake Status: {self.backend.emergencyBrake}")
        ...
        self.left_door_label = QLabel(f"Left Door Status: {self.backend.leftDoorStatus}")
        self.right_door_label = QLabel(f"Right Door Status: {self.backend.rightDoorStatus}")
        self.lights_status_label = QLabel(f"Lights Status: {self.backend.lights_status}")
        self.temperature_label = QLabel(f"Temperature: {self.backend.temperature}")
        ...
        for label in [self.speed_label, self.authority_label, self.currentSpeed_label, self.sbrake_label, self.ebrake_label, self.Kp_Label, self.Ki_Label, self.left_door_label, self.right_door_label, self.power_output_label, self.lights_status_label, self.temperature_label, self.hl_status_label, self.speed_limit_label]:
    ...
    def update_values(self):
        ...
        authority = self.backend.get_authority()
        sbrake = self.backend.getServBrakeStatus()
        ebrake = self.backend.getEBrakeStatus()
        currentSpeed = self.backend.get_currentSpeed()
        self.backend.set_power_output()
        power_output = self.backend.get_power_output()
        left_door_status = self.backend.getLeftDoorStatus()
        right_door_status = self.backend.getRightDoorStatus()
        lights_status = self.backend.getInternalLightStatus()
        internal_temperature = self.backend.get_temp()
        ...
        self.speed_label.setText(f"Commanded Speed: {commanded_speed}")
        self.currentSpeed_label.setText(f"Current Speed: {currentSpeed}")
        self.authority_label.setText(f"Authority: {authority}")
        self.sbrake_label.setText(f"Service Brake Status: {'ON' if sbrake else 'OFF'}")
        self.ebrake_label.setText(f"Emergency Brake Status: {'ON' if ebrake else 'OFF'}")
        self.Kp_Label.setText(f"Kp: {Kp}")
        self.Ki_Label.setText(f"Ki: {Ki}")
        self.power_output_label.setText(f"Power Output: {power_output}")
        self.left_door_label.setText(f"Left Door Status: {'Open'if left_door_status else 'Closed'}")
        self.right_door_label.setText(f"Right Door Status: {'Open'if right_door_status else 'Closed'}")
