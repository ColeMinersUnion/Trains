...
class TestbenchUI(QWidget):
    ...
    def submit_from_TM(self, commanded_speed, full_authority, brake_status, currentSpeed,  door_status, lights_status, internal_temp, headlights_status, speed_limit, beacon):
        cs = commanded_speed
        auth = full_authority
        self.backend.add_to_authority(auth, self.backend.full_authority, self.backend.curr_authority)
        ...
        curr_speed = currentSpeed
        door = door_status
        lights = lights_status
        temp  = internal_temp
        head = headlights_status
        speedlim = speed_limit
        beacon = beacon
        self.backend.update_testbench_status(cs, auth, brake, curr_speed, door, lights, temp, head, speedlim, beacon)
        print("updated from combined in testbench")
    ...
    def toggle_change_blocks(self):
        self.backend.block_switch = not self.backend.block_switch
        print("block switch")
        #self.backend.prev_block = not self.backend.prev_block
        self.backend.next_authority(self.backend.curr_authority, self.backend.block_switch, self.backend.prev_block)
