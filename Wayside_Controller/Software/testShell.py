#this file is to be used exclusively for testing shell communications with plc
#This will ensure that the main shell code in SoftwareShell.py stays untouched
import sys
import os
...
#replace QMainWindow with QObject to test only the update_occupancy function
#and then comment out all UI stuff
class WaysideShell(QObject):
    ...
    def update_occupancy(self, new_occupancy): 
        ...
        return self.occupancy, self.authority,self.switch_77,self.switch_85,self.switch_28,self.switch_13,self.signal_77,self.signal_85,self.signal_28,self.signal_13,self.crossing_19,self.crossing_108
    ...
    def update_ui(self):
        ...
        for i in range(1,150):
            self.wayside_block_table.setItem(i-1,0, QTableWidgetItem(str(self.occupancy[i])))

        for i in range(1,150):
            self.wayside_block_table.setItem(i-1,1, QTableWidgetItem(str(self.authority[i])))
        
        #for i in range(69, 150):
            #self.wayside_block_table.setItem(i-69,0, QTableWidgetItem(str(self.occupancy[i])))
        
        #for i in range(69, 150):
            #self.wayside_block_table.setItem(i-69,1, QTableWidgetItem(str(self.authority[i])))
